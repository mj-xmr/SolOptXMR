...
sudo apt install at ethtool wakeonlan
...
inside the file enter the below file, again, assuming that `enp2s0` is your interface :
...
IMO, it's however better to preserve your hardware, rather than receiving a few picos more for having to settle with chips being melt down.
